        ...
        <div className="mt-10">
            <div className="grid grid-cols-3 gap-6 p-6 mb-10 ">
                <div className="col-span-2 max-h-96">
                    ...
                    {blogs[0] && (
                        <div onClick={() => handleNavigateToSeeMorePage(blogs[0])} className="overflow-hidden relative mt-4 rounded-lg bg-gray-100 shadow-md">
                            <img
                            ...
                            <div onClick={() => handleNavigateToSeeMorePage(blogs[0])} className="rotate-[-45deg] absolute bottom-4 right-4 bg-white bg-opacity-70 rounded-full p-2 mr-2 mb-2 cursor-pointer">
                                <FaCircleArrowRight className="w-8 h-8" />
                            </div>
                        </div>
                    )}
                </div>
                <div onClick={() => handleNavigateToSeeMorePage(blogs[1])} className="grid grid-rows-2 gap-4 mt-16 ">
                    {blogs[1] && (
                        <div className="rounded-lg bg-white shadow-md">
                            ...
                            <a onClick={() => handleNavigateToSeeMorePage(blogs[1])} className="absolute top-64 mt-20 right-20 text-base font-bold bg-light_gray pl-2 pr-2 rounded-full hover:bg-primary text-black">Xem thêm</a>
                        </div>
                    )}
                    {blogs[2] && (
                        <div onClick={() => handleNavigateToSeeMorePage(blogs[2])} className="rounded-lg bg-white shadow-md">
                            <img
                                className="rounded-xl w-full h-full object-cover"
                                src={blogs[2]?.image}
                                alt="Image 2"
                            ...
                            <span onClick={() => handleNavigateToSeeMorePage(blogs[2])} className="absolute bottom-20 mt-20 right-20 text-base font-bold bg-light_gray pl-2 pr-2 rounded-full hover:bg-primary text-black">
                                Xem thêm
                            </span>
                    ...
                    <div className="flex items-start p-4 bg-white shadow-lg rounded-lg mb-4">
                        ...
                        <div className="flex flex-col justify-between">
                            <div>
                                ...
                                <strong className="block text-lg font-semibold mt-1">{blog.title}</strong>
                            </div>
                            <span onClick={() => handleNavigateToSeeMorePage(blog)} className="text-blue  cursor-pointer hover:underline mt-2">Xem thêm</span>
                        </div>
            ...
            <div className="flex justify-center mt-6">
                <button
                    ...
                    className="px-4 py-2 mx-2 bg-gray-300 rounded-md hover:bg-gray-400 disabled:opacity-50"
                >
                    Previous
                </button>
