...
export const Header = () => {
  ...
  return (
    <div className={`mb-7 ${showHeader ? "header-visible" : "header-hidden"}`}>
      ...
      >
        <Link to="/home">
          <p className="text-primary font-bold logo-text pt-2 flex items-center">
            FuHub <FaCampground className="ml-2 size-8" />
          </p>
        </Link>
        <ul className="navbar-menu flex text-gray normal-text pt-2">
          <li className="mr-10 dropdown">
            <Link to={""}>
              <span className="dropbtn">Trang chủ</span>
            </Link>
          </li>
          <li className="mr-10">
            <Link to={"/time-line"}>
              <span>Hành trình thành công</span>
            </Link>
          </li>
          <li className="mr-10">
            <Link to={"/blogs"}>
